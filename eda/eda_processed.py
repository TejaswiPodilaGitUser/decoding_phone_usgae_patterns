...
def main():
    ...
    if st.checkbox('Show Raw Data', key="show_raw_data_eda_processed"):
        st.write(df.head())
    ...
    with col1:
        ...
        plot = plots.plot_app_usage_trends()
        st.pyplot(plot)

    # Plot 2: Screen Time vs Data Usage
    with col2:
        st.subheader('Screen Time vs Data Usage')
        plot = plots.plot_screen_time_vs_battery()
        st.pyplot(plot)
    ...
    with col1:
        ...
        plot = plots.plot_battery_correlation()
        st.pyplot(plot)

    # Plot 4: Primary Use Distribution
    with col2:
        st.subheader('Primary Use Distribution')
        plot = plots.plot_primary_use_patterns()
        st.pyplot(plot)
    ...
    with col1:
        ...
        plot = plots.plot_data_usage_by_primary_use()
        st.pyplot(plot)
    ...
    with col1:
        ...
        plot = plots.plot_monthly_recharge_cost()
        st.pyplot(plot)

    # Plot 8: E-commerce Spend by Primary Use
    with col2:
        st.subheader('E-commerce Spend by Primary Use')
        plot = plots.plot_ecommerce_spend_by_primary_use()
        st.pyplot(plot)
